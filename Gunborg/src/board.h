...
static const uint64_t BLACK_SQUARES = A1 + A3 + A5 + A7 +
									  B2 + B4 + B6 + B8 +
									  C1 + C3 + C5 + C7 +
									  D2 + D4 + D6 + D8 +
									  E1 + E3 + E5 + E7 +
...
static const uint64_t WHITE_SQUARES = A2 + A4 + A6 + A8 +
									  B1 + B3 + B5 + B7 +
									  C2 + C4 + C6 + C8 +
									  D1 + D3 + D5 + D7 +
									  E2 + E4 + E6 + E8 +
...
inline int pop_count_sw(uint64_t b) {
	...
	#define lsb_to_square(b) __builtin_ctzll(b)*/
	#define lsb_to_square(b) __lzcnt64(b)
	#define msb_to_square(b) (63 - __lzcnt64(b))
	#define pop_count(b) _mm_popcnt_u64(b)
#else
